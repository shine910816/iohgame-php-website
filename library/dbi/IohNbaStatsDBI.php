...
class IohNbaStatsDBI
{
    ...
    public static function selectPlayerLastFiveStats($p_id, $game_season, $game_season_stage)
    {
        $dbi = Database::getInstance();
        $sql = "SELECT b.game_id," .
               ...
               " b.g_minutes_sec," .
               " b.g_points," .
               " b.g_field_goals_made," .
               " b.g_field_goals_attempted," .
               " b.g_three_points_made," .
               " b.g_three_points_attempted," .
               " b.g_free_throw_made," .
               " b.g_free_throw_attempted," .
               " b.g_rebounds," .
               " b.g_offensive_rebounds," .
               " b.g_defensive_rebounds," .
               " b.g_assists," .
               " b.g_steals," .
               " b.g_blocks," .
               " b.g_personal_fouls," .
               " b.g_turnovers" .
               " FROM g_nba_boxscore b" .
               " LEFT OUTER JOIN g_nba_schedule s ON s.game_id = b.game_id" .
               " WHERE b.game_season = " . $game_season .
               " AND b.game_season_stage = " . $game_season_stage .
               " AND b.p_id = " . $p_id .
               " ORDER BY s.game_start_date DESC" .
               " LIMIT 0,5";
        $result = $dbi->query($sql);
        if ($dbi->isError($result)) {
            $result->setPos(__FILE__, __LINE__);
    ...
    {
        $dbi = Database::getInstance();
        $sql = "SELECT game_season, game_season_stage, t_id," .
               " COUNT(*) AS gp," .
               " SUM(g_minutes) AS min," .
               " SUM(g_minutes_sec) AS sec," .
               " SUM(g_points) AS pts," .
               ...
               " AND game_season_stage != " . $game_season_stage .
               " AND game_season < " . $game_season .
               " AND game_season > " . ($game_season - 4) .
               " GROUP BY game_season, game_season_stage, t_id" .
               " ORDER BY game_season DESC";
        $result = $dbi->query($sql);
        if ($dbi->isError($result)) {
            $result->setPos(__FILE__, __LINE__);
            return $result;
        ...
        $data = array();
        while ($row = $result->fetch_assoc()) {
            $data[$row["game_season"]][$row["game_season_stage"]][$row["t_id"]] = $row;
        }
        $result->free();
        return $data;
    }
